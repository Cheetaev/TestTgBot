...
namespace TgBot
{
    ...
    public class BotFlow
    {
        ...
        private async Task Update(ITelegramBotClient bot, Update update, CancellationToken token)
        {
            ...
            {
                if (message.Text.ToLower().Contains("start monitoring11ww22qweqweqweqweq"))
                {
                    await bot.SendTextMessageAsync(message.Chat.Id, "File monitoring running test 111123232323232321");
